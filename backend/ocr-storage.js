...
function decodeEnvironmentKey(value) {
  ...
  if (key.length !== 32) {
    throw createStorageError('OCR_CONFIG_ENCRYPTION_KEY 必须是 32 字节 Base64 或 64 位十六进制字符串', 'INVALID_ENCRYPTION_KEY')
  }
  return key
}
...
function loadOrCreateLocalKey(filePath) {
  ...
  if (fs.existsSync(directoryPath)) {
    ...
    if (!directoryStats.isDirectory() || directoryStats.isSymbolicLink()) {
      throw createStorageError('OCR 本机密钥目录必须是普通目录', 'INVALID_LOCAL_KEY_DIRECTORY')
    }
  } else {
  ...
  if (process.platform !== 'win32') {
    ...
    const securedDirectoryStats = fs.lstatSync(directoryPath)
    const securedDirectoryMode = securedDirectoryStats.mode & 0o777
    if (
      !securedDirectoryStats.isDirectory() ||
      securedDirectoryStats.isSymbolicLink() ||
      securedDirectoryMode !== 0o700
    ) {
      throw createStorageError('OCR 本机密钥目录权限必须为 0700', 'INSECURE_LOCAL_KEY_DIRECTORY_MODE')
    }
  }

  if (!fs.existsSync(filePath)) {
    try {
      fs.writeFileSync(filePath, crypto.randomBytes(32), { flag: 'wx', mode: 0o600 })
    } catch (error) {
      if (error.code !== 'EEXIST') throw error
    }
  }

  const keyStats = fs.lstatSync(filePath)
  if (!keyStats.isFile() || keyStats.isSymbolicLink()) {
    throw createStorageError('本机 OCR 凭据加密密钥必须是普通文件', 'INVALID_LOCAL_KEY_FILE')
  }
  if (process.platform !== 'win32') {
    fs.chmodSync(filePath, 0o600)
    const securedMode = fs.lstatSync(filePath).mode & 0o777
    if (securedMode !== 0o600) {
      throw createStorageError('本机 OCR 凭据加密密钥权限必须为 0600', 'INSECURE_LOCAL_KEY_MODE')
    }
  }

  const key = fs.readFileSync(filePath)
  if (key.length !== 32) {
    throw createStorageError('本机 OCR 凭据加密密钥无效，请恢复原密钥或重新配置凭据', 'INVALID_LOCAL_KEY')
  }
  return key
}
...
function createOcrStorage({ db, keyFilePath, environmentKey } = {}) {
  ...
  const getEncryptionKey = () => {
    if (!encryptionKey) encryptionKey = loadOrCreateLocalKey(keyFilePath)
  ...
  const saveCredentials = db.transaction(({ apiKey, secretKey, expectedVersion }) => {
    ...
    if (row) {
      db.prepare(`
        UPDATE ocr_config
        SET ciphertext = ?, iv = ?, auth_tag = ?, version = version + 1, updated_at = datetime('now')
        WHERE id = 1
      `).run(encrypted.ciphertext, encrypted.iv, encrypted.authTag)
    } else {
      db.prepare(`
        INSERT INTO ocr_config (id, ciphertext, iv, auth_tag, version)
        VALUES (1, ?, ?, ?, 1)
      `).run(encrypted.ciphertext, encrypted.iv, encrypted.authTag)
    }
  ...
  const getConfigStatus = (credentials, source, storageError = false) => {
    const metadata = getCredentialMetadata()
    return {
      configured: Boolean(credentials?.apiKey && credentials?.secretKey),
  ...
  const insertHistoryStatement = db.prepare(`
    INSERT INTO ocr_history (
      request_id, status, amount, matched_text, words_count, recognized_text,
      error_code, http_status, error_message, duration_ms
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `)

  const insertHistory = (entry) => {
    const result = insertHistoryStatement.run(
      entry.requestId,
      entry.status,
      entry.amount ?? null,
      entry.matchedText || null,
      Number(entry.wordsCount) || 0,
      entry.recognizedText ? String(entry.recognizedText).slice(0, 100000) : null,
      entry.errorCode || null,
      entry.httpStatus || null,
      entry.errorMessage ? String(entry.errorMessage).slice(0, 1000) : null,
      Math.max(0, Math.round(Number(entry.durationMs) || 0))
    )
  ...
  const listHistory = ({ page, pageSize }) => {
    ...
    const rows = db
      .prepare(`
        SELECT id, request_id, status, amount, matched_text, words_count,
               error_code, http_status, error_message, duration_ms, created_at
        FROM ocr_history
        ORDER BY id DESC
        LIMIT ? OFFSET ?
      `)
  ...
  const getHistoryById = (id) => {
    const row = db.prepare('SELECT * FROM ocr_history WHERE id = ?').get(id)
    return mapHistoryRow(row, true)
  }
...
module.exports = { createOcrStorage }
