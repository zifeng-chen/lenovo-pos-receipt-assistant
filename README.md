...
- 单张图片最大允许 20MB。
- 图片使用浏览器原生 `object-fit: contain` 按固有宽高比缩放，不会被拉伸变形；在 1080P 屏幕的 125% 和 150% 浏览器缩放下仍保持相同比例与上下对齐。
...
├── .kiro/
│   └── steering/
│       └── auto-document-and-push.md # 自动更新文档、记录变更并推送的项目规则
├── .gitignore                 # Git 忽略规则
├── .nvmrc                     # 推荐 Node.js 版本
...
- 每次代码、配置或文档修改完成后，先运行与改动相关的构建、检查或冒烟验证。
- 功能、使用方法、部署方式、接口、数据结构或排版行为变化时，同步更新本 README。
- 每次修改都在 [`CHANGELOG.md`](./CHANGELOG.md) 当天日期下记录用户可感知的更新内容。
- 验证通过且不存在敏感文件、冲突或强制推送风险时，自动创建新提交并推送当前远程分支。
- 自动化约定保存在 [`.kiro/steering/auto-document-and-push.md`](./.kiro/steering/auto-document-and-push.md)。如果当前请求明确要求不提交或不推送，以当前请求为准。
...
3. 图片读取完成后会立即显示在 A4 预览中。两图齐全后，页面自动生成临时 JPEG 合成图并开始识别金额。
...
```text
票据YYYYMMDDHHmmss.png
```

例如 `票据20260818153045.png`。时间使用点击下载瞬间的门店电脑本地年月日时分秒。
...
请求体为完整 JPEG 或 PNG 二进制合成图，不使用 JSON 或 Base64 JSON。前端默认提交目标宽度 `1600px`、质量 `0.88` 的 JPEG。
...
后端先从内存缓存中获取百度 access token，然后调用通用文字识别，只接受与“实收金额、实付金额、应收金额、价税合计、合计、总计、TOTAL”等标签紧邻或紧邻下一行的唯一金额。标签附近存在多个无法消歧的数字，或只有无标签小数时，接口按 `422` 拒绝猜测。返回金额必须由用户在保存销售记录前核对。

限制和错误：

| 状态码 | 说明 |
| --- | --- |
| `400` | 请求体不是有效的 PNG/JPEG |
| `413` | 原始图片超过 6MB，或百度要求的 Base64 URL 编码结果超过 8MB |
| `422` | OCR 成功但未找到明确金额 |
| `429` | 单 IP 每分钟超过 10 次，或全局同时识别超过 2 个请求 |
| `502` | 百度鉴权、识别或网络响应失败 |
